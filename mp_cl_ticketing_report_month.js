   ...
   function(error, runtime, search, url, record, format, email, currentRecord) {
     ...
     function overviewChart(date_from, date_to) {
       ...
       mpexUsageResults.run().each(function(mpexUsageSet) {
         ...
         if (old_date == null) {
           ...
           total_usage = source_manual + source_shopify + source_portal +
             source_bulk;
       ...
       date_set_created.forEach(function(date) {
         ...
         var link2 =
           '<a href=\'https://1048144.app.netsuite.com/app/site/hosting/scriptlet.nl?script=1289&deploy=1&compid=1048144&custparam_params={"date_from":"' +
           getMonthStart(date) + '","date_to":"' + getMonthEnd(date) +
           '"}\' target=_blank>VIEW (per week)</a> ';
         var link3 =
         ...
         tableDataSet.push([link, dateCreated2DateSelectedFormat(date),
           created, closed, progress, open, cust, zee
         ]);
       });
     ...
     function sourceChart(date_from, date_to) {
       ...
       var title = 'Sender vs Receiver Breakdown (' + getFirstDay() + ' - ' +
         getLastDay() + ')';
       if (!isNullorEmpty(date_from) && !isNullorEmpty(date_to)) {
         var title = 'Sender vs Receiver Breakdown (' + date_from + ' - ' +
           date_to + ')';
