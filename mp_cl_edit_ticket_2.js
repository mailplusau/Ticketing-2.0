...
define(['N/error', 'N/runtime', 'N/search', 'N/url', 'N/record', 'N/format',
  ...
  function(error, runtime, search, url, record, format, email, currentRecord) {
    ...
    function pageInit() {
      ...
      $(document).ready(function() {
        selector_list.forEach(function(selector) {
          ...
          switch (selector) {
            case 'barcodes':
              ...
              var columnDefs = [{
                ...
                checkboxes: {
                  selectRow: true,
                  selectAllCallback: function(nodes, flag,
                    inderminate) {
                    console.log("Select all callback");
                    var table_barcodes = $(
                      '#tickets-preview-barcodes').DataTable();
                  ...
                  selectCallback: function(nodes, flag) {
                    ...
                    if (flag) {
                      $(nodes).closest('tr').addClass(
                        'selected');
                    } else {
                      $(nodes).closest('tr').removeClass(
                        'selected');
                    }
                  }
                }
                ...
                render: function(data, type, row, meta) {
                  ...
                  if (status === "Closed" || status ===
                    "In progress - IT" || !has_mpex_contact) {
                    data = '';
                  }
                  return data;
                },
                ...
                checkboxes: {
                  ...
                  selectAllCallback: function(nodes, flag,
                    ...
                    var rows = table_barcodes.rows().nodes().to$();
                    $.each(rows, function(index) {
                      if ($(rows[index]).hasClass(
                          'ignoreme')) {
                        $(rows[index]).removeClass(
                          'selected');
                      }
                    });
                ...
                render: function(data, type, row, meta) {
                  ...
                  if (data[9] == "Open") {
                    ...
                    "In Progress - Customer Service") {
                    var button_style = 'btn-warning';
                  } else {
                    var button_style = 'btn-danger';
                  }
                  return '<button class="btn ' + button_style +
                    ' edit_class glyphicon ' + icon +
            ...
            case 'invoices':
              ...
              var columnDefs = [{
                ...
                render: function(data, type, row, meta) {
                  ...
                  return '<button class="btn ' + button_style +
                    ' btn - sm edit_class glyphicon ' + icon +
                    '" type="button" data-toggle="tooltip" data-placement="right" title="' +
                    title + '"></button>';
                }
              }];
          ...
          $(table_id + ' thead tr').clone(true).appendTo(table_id +
            ' thead');
          $(table_id + ' thead tr:eq(3) th').each(function(i) {
            ...
            if (title == '') {
              ...
              $(this).html(
                '<input style="width: 90%" type="text" placeholder="Search ' +
                title + '" />');
      ...
      $.each(rows, function(index) {
        if (status[index] == "Closed" || status[index] ==
          "In progress - IT" || !has_mpex_contact[index]) {
      ...
      $('.table').each(function() {
        ...
        table.on('click', '.edit_class', function() {
          ...
          switch (selector) {
            case 'barcodes':
              var ticket_id = $(this).parent().siblings().eq(2).text()
                .split('MPSD')[1];
              var selector_number = $(this).parent().siblings().eq(4)
                .text();
            ...
            case 'invoices':
              var ticket_id = $(this).parent().siblings().eq(0).text()
                .split('MPSD')[1];
              var selector_number = $(this).parent().siblings().eq(2)
                .text();
            ...
            case 'customers':
              ...
              console.log(ticket_id + "," + selector_number + "," +
                selector_type);
              break;
          }
          ...
          if (isNullorEmpty(selector_number.trim())) {
            ...
            selector_number = ticketRecord.getValue({
              fieldId: 'altname'
            });
          }
    ...
    function onBulkEscalate() {
      ...
      currRec.setValue({
        fieldId: 'custpage_bulk_escalate',
        value: param_selected_ticket_id
      });
    ...
    function loadMpexContactSet() {
      ...
      ticketFilterExpression.push('AND', [
        ["custrecord_barcode_number", "noneof", "@NONE@"], "OR", ["name",
          "startswith", "MPE"
        ]
      ]);
      ...
      if (!isNullorEmpty(resultTicketSlice)) {
        do {
          resultTicketSlice = ticketResultSet.getRange({
            start: slice_index * 1000,
            end: (slice_index + 1) * 1000
          });
          resultTicketSlice.forEach(function(ticketResult) {
            var customer_id = ticketResult.getValue(
              'custrecord_customer1');
      ...
      var mpexCustomersSearch = search.load({
        type: 'customer',
        id: 'customsearch_customer_mpex_contacts'
      });
      ...
      if (!isNullorEmpty(mpexCustomersResultSet)) {
        mpexCustomersResultSet.each(function(customerResult) {
          var cust_has_mpex_cont_cust_id = customerResult.getValue(
            "internalid");
      ...
      if (!isNullorEmpty(resultTicketSlice)) {
        do {
          ...
          resultTicketSlice.forEach(function(ticketResult) {
            ...
            var status_val = ticketResult.getValue(
              'custrecord_ticket_status');
            ...
            switch (ticket_type) {
              case 'barcode':
                ...
                // TOLL Issues
                var toll_issues = ticketResult.getText(
                  'custrecord_toll_issues');
                toll_issues = toll_issues.split(',').join('<br>');
                var connote_number = ticketResult.getValue({
                ...
                if (!isNullorEmpty(resolved_toll_issues)) {
                  resolved_toll_issues = 'Resolved : <br>' +
                    resolved_toll_issues.split(',').join('<br>');
                }
              ...
              case 'invoice':
                ...
                if (!isNullorEmpty(resolved_invoice_issues)) {
                  resolved_invoice_issues = 'Resolved : <br>' +
                    resolved_invoice_issues.split(',').join('<br>');
                }
            ...
            // MP Ticket Issues
            var mp_ticket_issues = ticketResult.getText(
              'custrecord_mp_ticket_issue');
            mp_ticket_issues = mp_ticket_issues.split(',').join('<br>');

            // Resolved MP Ticket Issues
            var resolved_mp_ticket_issues = ticketResult.getText(
              'custrecord_resolved_mp_ticket_issue');
            if (!isNullorEmpty(resolved_mp_ticket_issues)) {
              resolved_mp_ticket_issues = 'Resolved : <br>' +
                resolved_mp_ticket_issues.split(',').join('<br>');
            }
            ...
            var customer_name = ticketResult.getText(
              'custrecord_customer1');
            ...
            switch (ticket_type) {
              case 'barcode':
                if (status_val != 9) {
                  ...
                  ticketsDataSetArrays[0].push(['', '', ticket_id,
                    date_created, barcode_number, connote_number,
                    customer_name, franchise_name, owners, status,
                    toll_issues, mp_ticket_issues, has_mpex_contact
                  ]);
                }
                break;
              ...
              case 'invoice':
                if (ticketsDataSetArrays[1] != undefined) {
                  ticketsDataSetArrays[1].push([ticket_id, date_created,
                    invoice_number, customer_name, franchise_name,
                    owners, status, invoice_issues, mp_ticket_issues
                  ]);
                }
                break;
              ...
              case 'customer':
                if (ticketsDataSetArrays[2] != undefined) {
                  ticketsDataSetArrays[2].push([ticket_id, date_created,
                    issue_type, customer_name, franchise_name, owners,
                    status, mp_ticket_issues
                  ]);
                }
                break;
            }
    ...
    function dataTablePreview(selector) {
      var inlineQty = '<style>table#tickets-preview-' + selector +
      ...
      inlineQty += '<table id="tickets-preview-' + selector +
    ...
    function getTicketType(ticketResult) {
      ...
      if (!isNullorEmpty(barcode_number)) {
        ...
        if (ticket_name.match(re_invoice)) {
          return 'invoice';
        } else if (ticket_name == "Customer App" || ticket_name ==
          "Customer Portal" || ticket_name == "Update Label") {
          return 'customer';
        } else {
          return 'barcode';
        }
