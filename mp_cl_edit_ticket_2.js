...
define([
	...
	function pageInit() {
		...
		$.each(rows, function (index) {
			if (
				status[index] == "Closed" ||
				status[index] == "In progress - IT" ||
				!has_mpex_contact[index]
			) {
			...
			table.on("click", ".edit_class", function () {
				...
				switch (selector) {
					...
					case "operations":
						...
						console.log(
							ticket_id + "," + selector_number + "," + selector_type
						);
						break;
				...
				var customerInternal = ticketRecord.getValue({
	...
	function onBulkEscalate() {
		...
		selected_tickets_id = selected_tickets_id.map(function (ticket_number) {
	...
	function loadMpexContactSet() {
		...
		ticketFilterExpression.push("AND", [
			["custrecord_barcode_number", "noneof", "@NONE@"],
			"OR",
			["name", "startswith", "MPE"],
		]);
		...
		mpexCustomersSearch.filters.push(
			search.createFilter({
				name: "internalid",
				join: null,
				operator: "anyof",
				...
				resultTicketSlice.forEach(function (ticketResult) {
					...
					switch (ticket_type) {
						...
						case "operations":
							...
							// TOLL Issues
							var operation_issues = ticketResult.getText(
								"custrecord_mp_ticket_mp_ops_issue"
							);
							operation_issues = operation_issues.split(",").join("<br>");

							// Resolved TOLL Issues
							var resolved_operation_issues = ticketResult.getText(
								"custrecord_resolved_mp_ops_issues"
							);
							if (!isNullorEmpty(resolved_operation_issues)) {
								resolved_operation_issues =
									"Resolved : <br>" +
									resolved_operation_issues.split(",").join("<br>");
							}
	...
	function getTicketType(ticketResult) {
		...
		if (!isNullorEmpty(barcode_number)) {
			...
			if (ticket_name.match(re_invoice)) {
				...
				ticket_name == "Customer App" ||
				ticket_name == "Customer Portal" ||
				ticket_name == "Update Label"
			) {
				return "customer";
