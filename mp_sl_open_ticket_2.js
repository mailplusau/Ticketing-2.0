...
define(['N/ui/serverWidget', 'N/email', 'N/runtime', 'N/search', 'N/record', 'N/http', 'N/log', 'N/redirect', 'N/format', 'N/file'],
    function (ui, email, runtime, search, record, http, log, redirect, format, file) {
                ...
                if (!isNullorEmpty(params)) {
                    ...
                    if (!isNullorEmpty(params.selector_number) && !isNullorEmpty(params.selector_type)) {
                        ...
                        if (!isNullorEmpty(params.ticket_id)) {
                            ...
                            escalated_to_it = ticketRecord.getValue({ fieldId: 'custrecord_date_escalated_it' });
                            ...
                            if (!isNullorEmpty(customer_barcode_number)) {
                                ...
                                senderEmail = rec.getValue({ fieldId: 'custrecord_sender_email' });
                                senderPhone = rec.getValue({ fieldId: 'custrecord_senders_phone' });
                                senderAddress = rec.getValue({ fieldId: 'custrecord_sender_address_1' });
                                productName = rec.getText({ fieldId: 'custrecord_cust_prod_stock_single_name' });
                                if (isNullorEmpty(productName)) {
                                    productName = rec.getText({ fieldId: 'custrecord_cust_stock_prod_name' })
                                }
                                ...
                                barcodeintegration = rec.getValue({ fieldId: 'custrecord_integration' });
                                barcodesource = rec.getText({ fieldId: 'custrecord_barcode_source' });
                                connote_number = rec.getValue({ fieldId: 'custrecord_connote_number' });
                            }
        ...
        function externalBarcodeSource(selector_type, barcodeintegration, barcodempdl, barcodesource, productName) {
            ...
            if (selector_type == 'barcode_number') {
                ...
                inlineQty += '<span class="input-group-addon" id="barcodempdl_text">Product Type</span>';
                inlineQty += '<input id="productName" value="' + productName + '" class="form-control productName" ' + disabled + ' />';
                inlineQty += '</div></div>';
                inlineQty += '</div></div>';
                ...
                inlineQty += '<div class="col-xs-4 barcodempdl_div">';
                inlineQty += '<div class="input-group">';
                inlineQty += '<span class="input-group-addon" id="barcodempdl_text">INTEGRATION</span>';
                inlineQty += '<input id="barcodempdl" value="' + barcodeintegration + '" class="form-control barcodempdl" ' + disabled + ' />';
                inlineQty += '</div></div>';
        ...
        function tabsSection(customer_number, ticket_id, selector_number, selector_id, selector_type, status_value, customer_name, daytodayphone, daytodayemail, franchisee_name, zee_main_contact_name, zee_email, zee_main_contact_phone, zee_abn, date_stock_used, time_stock_used, final_delivery_text, selected_enquiry_status_id, attachments_hyperlink, owner_list, list_toll_issues, list_resolved_toll_issues, comment, date_created, creator_id, creator_name, status, customer_id, accountsphone, accountsemail, zee_id, list_enquiry_mediums, total_enquiry_count, chat_enquiry_count, phone_enquiry_count, email_enquiry_count, selected_label_id, maap_bank_account_number, maap_parent_bank_account_number, account_manager, list_toll_emails, customer_barcode_number, connote_number, receiveremail, receiverphone, receivername, receiverstate, receiverzip, receiversuburb, receiveraddr1, receiveraddr2, prod_stock_invoice, barcodempdl, barcodesource, list_mp_ticket_issues, list_resolved_mp_ticket_issues, list_invoice_issues, list_resolved_invoice_issues, selected_invoice_method_id, accounts_cc_email, mpex_po_number, customer_po_number, selected_invoice_cycle_id, terms, customer_terms, screenshot_file, browser, login_email_used, operating_system, phone_used, old_sender_name, old_sender_phone, phone_interaction_count, email_interaction_count, escalated_to_it, senderName, senderAddress, senderEmail, senderPhone, productName, barcodeintegration, portal_enquiry_count) {
            ...
            // BUTTONS
            if (!isNullorEmpty(ticket_id)) {
                if (isTicketNotClosed(status_value) && selector_type != "customer_issue") {
            ...
            } else if (selector_type == "customer_issue" && isTicketNotClosed(status_value) && !isNullorEmpty(ticket_id) && status_value == 17) {
            ...
            if (!isNullorEmpty(ticket_id)) {
                ...
                inlineQty += externalBarcodeSource(selector_type, barcodeintegration, barcodempdl, barcodesource, productName)
            }
            ...
            if (!isNullorEmpty(ticket_id)) {
                inlineQty += senderDetails(selector_type, senderName, senderEmail, senderPhone, senderAddress);
         ...
         * @return  {String}    inlineQty
         */
        function ticketSection(date_created, creator_id, creator_name, status, connote_number) {
            ...
            inlineQty += '<div class="col-xs-6 status">';
            inlineQty += '<div class="input-group">';
            inlineQty += '<span class="input-group-addon" id="status_text">CONNOTE NUMBER</span>';
            inlineQty += '<input id="status" value="' + connote_number + '" class="form-control status" disabled />';
        ...
        * @return  {String}    inlineQty
        */
        function senderDetails(selector_type, senderName, senderEmail, senderPhone, senderAddress) {

            var disabled = 'disabled';

            if (isNullorEmpty(senderName) && isNullorEmpty(senderEmail) && isNullorEmpty(senderPhone) && isNullorEmpty(senderAddress)) {
                disabled = '';
            }
            var inlineQty = '';
            ...
            if (selector_type == 'barcode_number') {
                ...
                inlineQty += '<div class="input-group">';
                inlineQty += '<span class="input-group-addon" id="sendername_text">NAME</span>';
                inlineQty += '<input id="sendername" value="' + senderName + '" class="form-control sendername" ' + disabled + ' />';
                inlineQty += '</div></div>';
                ...
                inlineQty += '<div class="input-group">';
                inlineQty += '<span class="input-group-addon" id="senderaddress_text">ADDRESS</span>';
                inlineQty += '<input id="senderaddress" value="' + senderAddress + '" class="form-control senderaddress" ' + disabled + ' />';
                inlineQty += '</div></div>';
        ...
        */
        ...
        function enquiryCountSection(total_enquiry_count, chat_enquiry_count, phone_enquiry_count, email_enquiry_count, selector_type, phone_interaction_count, email_interaction_count, portal_enquiry_count) {
            ...
            if (selector_type == "customer_issue") {
                inlineQty += '<div class="form-group container interaction_count_breakdown_section">';
                inlineQty += '<div class="row">';
        ...
        function isTicketNotClosed(status_value) {
            var is_ticket_not_closed = ((status_value != 3) && (status_value != 8) && (status_value != 15)) ? true : false;
