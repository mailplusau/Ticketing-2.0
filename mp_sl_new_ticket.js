...
define(['N/ui/serverWidget', 'N/email', 'N/runtime', 'N/search', 'N/record',
  ...
  function (ui, email, runtime, search, record, https, log, redirect, format,
    ...
    function onRequest(context) {
      ...
      if (!isNullorEmpty(tracking_number) && !isNullorEmpty(sender_or_receiver) && !isNullorEmpty(first_name) && !isNullorEmpty(last_name) && !isNullorEmpty(form_email) && !isNullorEmpty(phone) && !isNullorEmpty(company_name) && !isNullorEmpty(issues)) {
        ...
        if (!isNullorEmpty(ticketIdIfExists)) {
          ...
          var customer_id = ticketRecord2.getValue({
            fieldId: 'custrecord_customer1'
          });
          var ticket_name = ticketRecord2.getValue({
            fieldId: 'name'
          });
          barcodeRecordId = ticketRecord2.getValue({
            fieldId: 'custrecord_barcode_number'
          });
          sendCustomerTicketEmail('MailPlus [' + ticket_name +
            '] - Status Update - ' + tracking_number, [form_email], 112,
            customer_id);
          ...
          var rec_addr1 = barcodeRecord.getValue({
            fieldId: 'custrecord_receiver_addr1'
          });
          var rec_addr2 = barcodeRecord.getValue({
            fieldId: 'custrecord_receiver_addr2'
          });
          var rec_state = barcodeRecord.getValue({
            fieldId: 'custrecord_receiver_state'
          });
          var rec_zip = barcodeRecord.getValue({
            fieldId: 'custrecord_receiver_postcode'
          });
          var rec_suburb = barcodeRecord.getValue({
            fieldId: 'custrecord_receiver_suburb'
          });
          var rec_name = barcodeRecord.getValue({
            fieldId: 'custrecord_receiver_name'
          });
          var rec_phone = barcodeRecord.getValue({
            fieldId: 'custrecord_receiver_phone'
          });
          var rec_email = barcodeRecord.getValue({
          ...
          if (sender_or_receiver == 2) {
            ...
            if (isNullorEmpty(rec_name)) {
              barcodeRecord.setValue({
                fieldId: 'custrecord_receiver_name',
                value: del_first_name + ' ' + del_last_name
              });
            }
            ...
            if (isNullorEmpty(rec_phone) && !isNullorEmpty(del_phone_number)) {
              barcodeRecord.setValue({
                fieldId: 'custrecord_receiver_phone',
                value: del_phone_number
              });
            }
          }

          //set receiver addr
          if (isNullorEmpty(rec_addr1) && isNullorEmpty(rec_addr2) &&
            isNullorEmpty(rec_state) && isNullorEmpty(rec_zip) && isNullorEmpty(
              rec_suburb)) {
          ...
          if ((isNullorEmpty(activeBarcodeResults))) {
            ...
            customer_id = activeSelectorResult.getValue(
            ...
            var zee_id = customerRecord.getValue({
              fieldId: 'partner'
            });
            ...
            var reminder_date = new Date(today_year, today_month,
              today_day_in_month + addNbDays);
          ...
          if (!isNullorEmpty(barcodeRecordId)) {
            ...
            //set receiver addr
            if (isNullorEmpty(rec_addr1) && isNullorEmpty(rec_addr2) &&
              isNullorEmpty(rec_state) && isNullorEmpty(rec_zip) &&
              isNullorEmpty(rec_suburb)) {
          ...
          if (sender_or_receiver == 1) {
            ...
            note += '\nReceiver Delivery Address: ' + addr1 + ', ' + addr2 +
              ', ' + city + ', ' + postcode + ', ' + state;
            ...
            note += '\nDelivery Address: ' + addr1 + ', ' + addr2 + ', ' + city +
              ', ' + postcode + ', ' + state;
          ...
          userNote.setValue({
            fieldId: 'title',
            value: 'MailPlus Customer - New Ticket Information'
          });
     ...
     */
    function sendCustomerTicketEmail(subject, recipients, template,
      customer_id) {
      ...
      suiteletUrl += template + '&recid=' + customer_id + '&salesrep=' +
        sales_rep + '&dear=' + '' + '&contactid=' + null + '&userid=' +
        userid;
